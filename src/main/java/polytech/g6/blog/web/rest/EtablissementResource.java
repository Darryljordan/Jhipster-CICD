...
/**
 * REST controller for managing {@link polytech.g6.blog.domain.Etablissement}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Etablissement> partialUpdateEtablissement(
        ...
        Optional<Etablissement> result = etablissementRepository
            ...
            .map(existingEtablissement -> {
                if (etablissement.getNomE() != null) {
                    existingEtablissement.setNomE(etablissement.getNomE());
                }
                if (etablissement.getAdresseE() != null) {
                    existingEtablissement.setAdresseE(etablissement.getAdresseE());
                }
